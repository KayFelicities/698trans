...
def Get0501(offset,*data_in):
    ...
    apdu_len += 1
    oad = data_in[offset + apdu_len : offset + apdu_len + 4]
    sFindDataID(oad)
    apdu_len=apdu_len + 4
    return apdu_len
...
def Get0502(offset,*data_in):
    ...
    output(data_in[offset + apdu_len] + ' —— SEQUENCE OF OAD，个数=' + str(oad_count))
    apdu_len += 1
...
def Get0503(offset,*data_in):
    ...
    if selector == 1:# 选择方法1
        sFindDataID(data_in[offset + apdu_len : offset + apdu_len + 4])
        apdu_len += 4
        apdu_len += HandleDateType(apdu_len + offset,1,1,*data_in)
    elif selector == 2:# 选择方法2
        sFindDataID(data_in[offset + apdu_len : offset + apdu_len + 4])
        apdu_len=apdu_len + 4
        tmpLenLen=HandleDateType(apdu_len + offset,1,3,*data_in)
        apdu_len=apdu_len + tmpLenLen
    elif selector == 3:# 选择方法3
        ...
        for k in range(0,SumData) :
            ...
            tmpLenLen=HandleDateType(apdu_len + offset,1,3,*data_in)
            apdu_len=apdu_len + tmpLenLen
    elif selector==4 or selector==5 :          #选择方法45
        apdu_len += take_date_time_s(data_in[offset + apdu_len :], '(采集启动时间)' if selector == 4 else '(采集存储时间)')
        apdu_len += take_MS(data_in[offset + apdu_len :])
    elif selector==6 or selector==7 or selector==8 :          #选择方法678
        AtmpText=';;'.split(';')
        AtmpText[0]='启动'
        AtmpText[1]='存储'
        AtmpText[2]='成功'

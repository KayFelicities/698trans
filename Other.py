...
def connect_request(offset,*data_in):
    ...
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 客户机发送帧最大尺寸:'+stXinTiao+'字节')
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 客户机接收帧最大尺寸:'+stXinTiao+'字节')
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len],16))
    output(data_in[offset+apdu_len]+' —— 客户机接收帧最大窗口尺寸:'+stXinTiao+'个')
    apdu_len=apdu_len+1
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 客户机最大可处理APDU尺寸:'+stXinTiao)
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1]+data_in[offset+apdu_len+2]+data_in[offset+apdu_len+3],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' '+data_in[offset+apdu_len+2]+' '+data_in[offset+apdu_len+3]+' —— 期望的应用连接超时时间:'+stXinTiao+'s')
    apdu_len=apdu_len+4
...
def connect_response(offset,*data_in):
    ...
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 服务器发送帧最大尺寸:'+stXinTiao+'字节')
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 服务器接收帧最大尺寸:'+stXinTiao+'字节')
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len],16))
    output(data_in[offset+apdu_len]+' —— 服务器接收帧最大窗口尺寸:'+stXinTiao+'个')
    apdu_len=apdu_len+1
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' —— 服务器最大可处理APDU尺寸:'+stXinTiao)
    apdu_len=apdu_len+2
    stXinTiao=str(int(data_in[offset+apdu_len]+data_in[offset+apdu_len+1]+data_in[offset+apdu_len+2]+data_in[offset+apdu_len+3],16))
    output(data_in[offset+apdu_len]+' '+data_in[offset+apdu_len+1]+' '+data_in[offset+apdu_len+2]+' '+data_in[offset+apdu_len+3]+' —— 商定的应用连接超时时间:'+stXinTiao+'s')
    apdu_len=apdu_len+4
    connect_result = {
        '00' : ' —— 允许建立应用连接',
        '01' : ' —— 密码错误',

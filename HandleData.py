...
def OADHandle(Num, *Text):
    ...
    output(Text[Num + 1] + ' ' + Text[Num + 2] + ' ' +
           Text[Num + 3] + ' ' + Text[Num + 4] + ' ——' + tmpText)
    return Num + 4
...
def HandleDate(Num, Sign, Id, *Text):
    ...
    if Sign != 0:
        for k in range(0, tt1):
            if (OI == (Dlist[k].decode('utf-8').split('=')[0])):
                output(Text[Num] + ' ——[' + Dlist[k].decode('utf-8').split('=')[0] +
                       '] ' + Dlist[k].decode('utf-8').split('=')[1])  # show data type
                break
    else:
        OI = Id
        Num -= 1
    ...
    if OI == 0:
        ...
        if OI == 4:  # bit-string
            ...
            output(bit_string_text + ' ——bit-stringn内容:' +
                   str(bin(int(bit_string_text.replace(' ', ''), 16))))
        ...
        if OI == 5 or OI == 6:  # double-long double-long-unsigned
            ...
            output(TempStr + ' ——数据(' + str(int(TempStr[9:11], 16) + int(TempStr[6:8], 16) * 256 + int(
                TempStr[3:5], 16) * 65536 + int(TempStr[0:2], 16) * 16777216) + ')')
            return Num + 1
        ...
        if OI == 16 or OI == 18:  # unsigned long-unsigned
            ...
            for k in range(0, 2):
                ...
                Num += 1
            output(
                TempStr + ' ——数据(' + str(int(TempStr[3:5], 16) + int(TempStr[0:2], 16) * 256) + ')')
            return Num + 1
        ...
        if OI == 20 or OI == 21:  # long64 long64-unsigned
            ...
            for k in range(0, 8):
                TempStr = TempStr + Text[Num + 1] + ' '
                Num += 1
            output(TempStr + ' ——数据(' + str(int(TempStr[0:2], 16) * 16777216 * 256 * 256 * 256 * 256 + int(TempStr[3:5], 16) * 16777216 * 256 * 256 * 256 + int(TempStr[6:8], 16) * 16777216 * 256 * 256 + int(
                TempStr[9:11], 16) * 16777216 * 256 + int(TempStr[12:14], 16) * 16777216 + int(TempStr[15:17], 16) * 65536 + int(TempStr[18:20], 16) * 256 + int(TempStr[21:23], 16)) + ')')
            return Num + 1
        ...
        if OI == 25:  # date_time
            ...
            if len(sec) != 2:
                sec = '0' + sec
            output(TempStr + ' ——' + year + '年' + month + '月' + day +
                   '日' + hour + ':' + fen + ':' + sec + ':' + minsec)
        ...
        if OI == 85:  # TSA
            ...
            output(str(Text[Num + 1]) + ' ' + str(Text[Num + 2]) +
                   '——TSA长度 ' + str(int(Text[Num + 2], 16) + 1))
            tsa_data = ''
        ...
        if OI == 92:  # MS
            ...
            if Text[Num + 1] == '02':  # unsigned
                ...
                output(Text[Num + 2] + ' —— SEQUENCE OF unsigned的个数 = ' +
                       str(int(Text[Num + 2], 16)))
                Tnum = Num + 3
            ...
            if Text[Num + 1] == '04':  # long-unsigned
                ...
                output(Text[Num + 2] + ' —— SEQUENCE OF long-unsigned的个数 = ' +
                       str(int(Text[Num + 2], 16)))
                Tnum = Num + 3
                for k in range(0, tNum):
            ...
            if Text[Num + 1] == '07':  # Region
                ...
                for k in range(0, tNum):
                    if Text[Tnum + 1] == '0C' and Text[Tnum + 4] == '0C':  # long_unsigned
                        ...
                        Tstr = Text[Tnum + 2] + ' ' + Text[Tnum + 3] + \
                            ' ' + Text[Tnum + 5] + ' ' + Text[Tnum + 6]
                        if Text[Tnum] == '00':
                            output(Text[Tnum] + ' ——( )')
                            Str = (Tstr + ' ——(' + Str1 + ',' + Str2 + ')')
                        elif Text[Tnum] == '01':

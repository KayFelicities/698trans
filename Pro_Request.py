...
def Pro0901(offset,*data_in) :
    ...
    output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' —— 整个代理请求的超时时间='+str(MaxTime)+'s')    
    ...
    for i in range(0,ListSum) :     
        ...
        output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' —— 代理一个服务器的超时时间='+str(SecSum)+'s')    
        tmpAPDULen=tmpAPDULen+2
...
def Pro0902(offset,*data_in) :
    ...
    MaxTime=int(data_in[offset+tmpAPDULen]+data_in[offset+tmpAPDULen+1],16)
    output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' —— 代理请求的超时时间='+str(MaxTime)+'s')    
    tmpAPDULen=tmpAPDULen+2
    ...
    if ChoiceWay==6 or ChoiceWay==7 or ChoiceWay==8 :          #选择方法678
        ...
        output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' '+data_in[offset+tmpAPDULen+2]+'—— 间隔')    
...
def Pro0907(offset,*data_in) :
    ...
    output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' '+data_in[offset+tmpAPDULen+2]+' '+data_in[offset+tmpAPDULen+3]+' '+data_in[offset+tmpAPDULen+4]+' —— 端口通信控制块')    
    tmpAPDULen=tmpAPDULen+5
    
    SecSum=int(data_in[offset+tmpAPDULen]+data_in[offset+tmpAPDULen+1],16)
    output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' —— 接收等待报文超时时间='+str(SecSum)+'s')    
    tmpAPDULen=tmpAPDULen+2    
    
    SecSum=int(data_in[offset+tmpAPDULen]+data_in[offset+tmpAPDULen+1],16)
    output(data_in[offset+tmpAPDULen]+' '+data_in[offset+tmpAPDULen+1]+' —— 接收等待字节超时时间='+str(SecSum)+'ms')    
    tmpAPDULen=tmpAPDULen+2        

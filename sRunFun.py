...
#长度
def ShowLen(*data_in) :
    output(data_in[1]+' '+data_in[2]+' —— 长度域L='+str(int(data_in[2]+data_in[1],16))+'字节')
    return 0
...
def lShowFenZen(offset,*data_in) :
    ...
    if frame_separation_flag == 1:
        ...
        output(data_in[offset]+' '+data_in[offset+1]+' —— 分帧序号:' + str(frame_separation_seq) + frame_separation_type)
        return 2
    else :
        return 0
...
def show_service_type(offset,*data_in) :
    ...
    if server_type_line != '':
        output(data_in[offset] + ' —— ' + server_type_line.decode('utf-8').split('=')[1].split('\n')[0])
        try:
            server_type2 = int(data_in[offset+1],16)
            output(data_in[offset + 1] + ' —— ' + server_type_line.decode('utf-8').split('=')[server_type2 + 2].split('\n')[0])
        except:
            pass
